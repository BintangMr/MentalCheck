...
@section('breadcrumb')
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i>
    <a href="{{ route('admin.question.answer',[$category_id, $soal_id]) }}" class="breadcrumb--active">List Jawaban</a>
@endsection

@section('subhead')
    <title>Mental Check | Admin Jawaban</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">List Jawaban</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <button class="btn btn-primary shadow-md mr-2" id="btn-create">Tambah Jawaban</button>
        </div>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        ...
        <div class="overflow-x-auto scrollbar-hidden">
            <div id="jawaban-table" class="mt-5 table-report table-report--tabulator"></div>
        </div>
    </div>
    <!-- END: HTML Table Data -->
@endsection
...
@push('js')
    <script>
        ...
        if (cash("#jawaban-table").length) {
            ...
            // Setup Tabulator
            var table = new Tabulator.default("#jawaban-table", {
                ajaxURL: '{{ route('admin.question.answer',[$category_id,$soal_id]) }}',
                ajaxFiltering: true,
                ajaxSorting: true,
                printAsHtml: true,
                printStyled: true,
                ...
                columns: [{
                    ...
                    formatter: function formatter(cell, formatterParams) {
                        var a = cash("<div class=\"flex lg:justify-center items-center\">\n" +
                            "<a class=\"edit flex text-theme-11 items-center mr-3\" href=\"{{ route('admin.question.answer.edit',[$category_id ,$soal_id,'']) }}/"+cell.getData().id+"\">\n" +
                        ...
                        cash(a).find(".delete").on("click", function () {
                            Swal.fire({
                                ...
                                text: "Anda akan menghapus data jawaban!",
                                icon: 'warning',
                                ...
                                if (result.isConfirmed) {
                                    axios.delete('{{ route('admin.question.answer.delete',[$category_id,$soal_id]) }}', {
                                        params: {
                                    ...
                                    Swal.fire(
                                        'Dibatalkan!',
                                        'Anda membatalkan penghapusan jawaban',
                                        'error'
                                    )
                                }
                            })
            ...
            cash("#tabulator-html-filter-reset").on("click", function (event) {
                cash("#tabulator-html-filter-field").val("answer");
        ...
        cash('#btn-create').on('click', function (event){
            window.location.href = '{{ route('admin.question.answer.create',[$category_id,$soal_id]) }}';
        });
    </script>
@endpush
