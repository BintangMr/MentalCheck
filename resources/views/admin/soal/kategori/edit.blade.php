...
@section('breadcrumb')
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i>
    <a href="{{ route('admin.question.category.create') }}" class="breadcrumb--active">Edit Ketegori Soal</a>
@endsection
...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Edit Kategori Soal : {{ $category->category }}</h2>
    </div>
    <!-- BEGIN: HTML Table Data -->
    <div class="intro-y box p-5 mt-5">
        <div id="vertical-form" class="p-5">
            <form id="form">
                <input id="id" value="{{ $category->id }}" style="display: none">
                <div>
                    <label for="kategori" class="form-label">Nama Kategori</label>
                    <input id="kategori" required type="text" class="form-control kategori__input"
                           placeholder="Nama Kategori" value="{{ $category->category }}">
                ...
                <div class="mt-3">
                    ...
                    <div class="intro-y grid grid-cols-12 sm:gap-6 gap-y-6 box px-5 py-8 mt-5">
                        <div class="col-span-6 sm:col-span-3 lg:col-span-2 xl:col-span-1 text-center">
                            ...
                            <input id="" class="form-check-input mt-2" {{ $category->icon == 'smile' ? 'checked' : '' }}
                        ...
                        <div class="col-span-6 sm:col-span-3 lg:col-span-2 xl:col-span-1 text-center">
                            ...
                            <input id="" class="form-check-input mt-2" {{ $category->icon == 'cloud-drizzle' ? 'checked' : '' }}
                        ...
                        <div class="col-span-6 sm:col-span-3 lg:col-span-2 xl:col-span-1 text-center">
                            ...
                            <input id="radio-switch-4" class="form-check-input mt-2" {{ $category->icon == 'minimize' ? 'checked' : '' }}
                ...
                <div class="mt-3">
                    ...
                    <textarea id="deskripsi" required type="text" class="form-control deskripsi__input"
                              placeholder="Deksripsi">{{ $category->description }}</textarea>
                    <div id="error-deskripsi" class="deskripsi__input-error w-5/6 text-theme-6 mt-2"></div>
                </div>
                <div class="mt-3">
                    <div class="mt-2">
                        <div class="form-check">
                            <input id="aktif" name="aktif" class="form-check-switch" type="checkbox" value="true" @if($category->status) checked @endif>
                            <label class="form-check-label" id="aktif-label" for="checkbox-switch-7">@if($category->status) Aktif @else Tidak Aktif @endif</label>
                        </div>
                    </div>
                </div>
                <div class="mt-3">
    ...
    <script>
        cash(function () {
            async function simpan() {
                ...
                axios.post('{{ route('admin.question.category.update',['id' => $category->id]) }}',formData, {
                    headers: {
            ...
            $("#background").fileinput({
                ...
                defaultPreviewContent: '<img src="{{ asset($category->image) }}" alt="Avatar"><h6 class="text-muted">Klik untuk mengganti</h6>',
