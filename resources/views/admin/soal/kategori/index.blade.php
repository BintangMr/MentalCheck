...
@section('breadcrumb')
    ...
    <a href="{{ route('admin.question.category') }}" class="breadcrumb--active">Kategori Soal</a>
@endsection

@section('subhead')
    <title>Mental Check | Admin Kategori Soal</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Kategori Soal</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <button class="btn btn-primary shadow-md mr-2" id="btn-create">Tambah Kategori</button>
        </div>
    </div>
    ...
    <script>
        ...
        if (cash("#kategori-table").length) {
            ...
            var table = new Tabulator.default("#kategori-table", {
                ajaxURL: '{{ route('admin.question.category') }}',
                ajaxFiltering: true,
                ajaxSorting: true,
                printAsHtml: true,
                printStyled: true,
                ...
                placeholder: "Tidak ada data kategori yang sesuai",
                ...
                columns: [{
                    ...
                    formatter: function formatter(cell, formatterParams) {
                        return "<div>\n" +
                            "<div class=\"font-medium whitespace-nowrap\">".concat(cell.getData().category, "</div>\n " +
                                "</div>");
                    }
                }, {
                    title: "Deksripsi",
                    ...
                    formatter: function formatter(cell, formatterParams) {
                        var a = cash("<div class=\"flex lg:justify-center items-center\">\n" +
                            "<a class=\"show flex items-center mr-3\" href=\"{{ route('admin.question','') }}/" + cell.getData().id + "\">\n" +
                            "<i data-feather=\"eye\" class=\"w-4 h-4 mr-1\"></i> Soal\n" +
                            "</a>\n" +
                            "<a class=\"hasil flex items-center mr-3\" href=\"{{ route('admin.question.result','') }}/" + cell.getData().id + "\">\n" +
                            "<i data-feather=\"lock\" class=\"w-4 h-4 mr-1\"></i> Hasil\n" +
                            "</a>\n" +
                            "<a class=\"edit flex text-theme-11 items-center mr-3\" href=\"{{ route('admin.question.category.edit','') }}/" + cell.getData().id + "\">\n" +
                        ...
                        cash(a).find(".delete").on("click", function () {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    axios.delete('{{ route('admin.question.category.delete','') }}/'+cash(this).data("id"), {
                                        params: {
        ...
        cash('#btn-create').on('click', function (event) {
            window.location.href = '{{ route('admin.question.category.create') }}';
        });
    </script>
@endpush
