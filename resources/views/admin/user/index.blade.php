...
@section('subhead')
    <title>Mental Check | Admin Users</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">User</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <button class="btn btn-primary shadow-md mr-2" id="btn-create">Tambah User</button>
        </div>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        ...
        <div class="overflow-x-auto scrollbar-hidden">
            <div id="user-table" class="mt-5 table-report table-report--tabulator"></div>
        </div>
    </div>
    <!-- END: HTML Table Data -->
@endsection
...
@push('js')
    <script>
        ...
        if (cash("#user-table").length) {
            ...
            // Setup Tabulator
            var table = new Tabulator.default("#user-table", {
                ajaxURL: '{{ route('admin.user') }}',
                ajaxFiltering: true,
                ajaxSorting: true,
                printAsHtml: true,
                printStyled: true,
                ...
                placeholder: "Tidak ada data user yang sesuai",
                ...
                columns: [{
                    ...
                    formatter: function formatter(cell, formatterParams) {
                        return "<div class=\"flex lg:justify-center\">\n  " +
                                    "<div class=\"intro-x w-10 h-10 image-fit\">\n" +
                                        "<img alt=\"Avatar\" class=\"rounded-full\" src=\"".concat(cell.getData().avatar, "\">\n" +
                                    "</div>\n" +
                                "</div>");
                    }
                },  {
                    ...
                    formatter: function formatter(cell, formatterParams) {
                        return "<div class=\"flex items-center lg:justify-center "
                            .concat(cell.getData().admin ? "text-theme-10" : "text-theme-9", "\">\n " +
                                "<i data-feather=\"check-square\" class=\"w-4 h-4 mr-2\"></i> ")
                            .concat(cell.getData().admin ? "Admin" : "Member", "\n " +
                                " </div>");
                    }
                }, {
                    title: "ACTIONS",
                    ...
                    formatter: function formatter(cell, formatterParams) {
                        var a = cash("<div class=\"flex lg:justify-center items-center\">\n" +
                            "<a class=\"show flex text-theme-10 items-center mr-3\" href=\"{{ route('admin.user.show',['']) }}/"+cell.getData().id+"\">\n" +
                            "<i data-feather=\"eye\" class=\"w-4 h-4 mr-1\"></i> Show\n" +
                            "</a>\n" +
                            "<a class=\"edit flex text-theme-11 items-center mr-3\" href=\"{{ route('admin.user.edit',['']) }}/"+cell.getData().id+"\">\n" +
                        ...
                        cash(a).find(".delete").on("click", function () {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    axios.delete('{{ route('admin.user.delete', ['']) }}/' + cash(this).data("id"), {
                                        params: {
